...
LLM_API_KEY_STRONG=your_strong_model_key  # 可选，用于强模型
...
def select_model(content, risk_score):
    if risk_score > 0.8:
        return "strong_model"  # 高风险用强模型
    elif len(content) > 2000:
        return "strong_model"  # 长文本用强模型
    else:
        return "light_model"  # 默认轻量模型
```
